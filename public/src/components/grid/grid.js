...
export default {
  ...
  computed: {
    ...
    imagesNotInUse() {
      if (
        !this.rimCells ||
        this.rimCells.length == 0 ||
        this.rimCells.map((c) => c.data).filter((c) => c !== null).length == 0
      )
        return this.RIM_IMAGES;
      return this.RIM_IMAGES.filter((image) => {
        return (
          !this.rimCells.some(
            (cell) => cell.data === image.data || cell.data === image.data2
          ) && this.centerCell.data !== image.data
        );
      });
    },
  },
  ...
  methods: {
    ...mapActions([
      "fetchRimImages",
      "fetchCenterImages",
      "incrementOuterImageClickThruCountById",
      "incrementCenterImageClickThruCountById",
    ]),
    async setupGrid() {
      this.loading = true;
      ...
      if (this.usingLocalData || this.getPlatform === null) {
        ...
        if (this.getPlatform !== null) {
          const foundImage = this.CENTER_IMAGES.find(
            (image) => image._id === this.getPlatform.image_id
          );
          if (foundImage)
            this.centerCell = {
              ...foundImage,
              clickToClickThroughCount: this.clickToClickThroughCount,
              isMainCell: true,
            };
    ...
    async outerCellClicked(cell, index) {
      ...
      if (cell._id !== this.centerCell._id)
        await this.incrementOuterImageClickThruCountById(cell._id);
    ...
    cellClick(cell) {
      if (
        (cell.data2 && cell.data2 !== cell.hyperLink) ||
    ...
    async mainCellClick(cell) {
      let shouldIncCenterClickThru = !(
        ...
        this.clickToClickThroughCount > 0 &&
        cell.data &&
        (cell.data.isEmbed || cell.data.isWidget)
      );
      if (shouldIncCenterClickThru)
        await this.incrementCenterImageClickThruCountById(this.centerCell._id);
    ...
    getRandomCenterCellImage() {
      let possibleCenterImages = [
        ...this.CENTER_IMAGES.filter((image) =>
          this.rimCells.some((cell) => image.data === cell.data)
        ),
      ];
      if (
        !this.rimCells ||
        this.rimCells.length === 0 ||
        !possibleCenterImages ||
        possibleCenterImages.length === 0
      )
        possibleCenterImages = [...this.CENTER_IMAGES];
      let randomIdx = Math.floor(Math.random() * possibleCenterImages.length);
      return {
        ...possibleCenterImages[randomIdx],
        clickToClickThroughCount: this.clickToClickThroughCount,
        isMainCell: true,
      };
    },
    getRandomCenterCellWidget() {
      ...
      return {
        ...Images.centerWidgets[randomIdx],
        clickToClickThroughCount: this.clickToClickThroughCount,
        isMainCell: true,
      };
    },
    ...
    moveGridClockwise() {
      ...
      for (let i = 0; i < this.rimCount; i++) {
        let prevIndex =
          i == 0
            ? clockWiseIndexOrder[clockWiseIndexOrder.length - 1]
            : clockWiseIndexOrder[i - 1];
        let currIndex = clockWiseIndexOrder[i];

        if (currIndex == 0 && this.rimCells[prevIndex].startingIndex == 0)
          this.revolutions++;

        const currentPositionIndex = parseInt(
          Object.keys(this.rimCellPositions).find((key) => {
            const currentCellPosition = this.rimCells[currIndex].position;
            const keyPosition = this.rimCellPositions[key];
            return (
              keyPosition.x == currentCellPosition.x &&
              keyPosition.y == currentCellPosition.y
            );
          })
        );
        const nextPositionIndex =
          currentPositionIndex == this.rimCount ? 1 : currentPositionIndex + 1;
        this.rimCells[currIndex].position = this.rimCellPositions[
          nextPositionIndex
        ];
    ...
    randomizeCenterCell() {
      this.clickToClickThroughCount = 0;
      this.centerCell = this.usingLocalData
        ? { ...this.getRandomCenterCellWidget(), startingIndex: null }
        : { ...this.getRandomCenterCellImage(), startingIndex: null };
    },
    resizeGrid() {
    ...
    startMovingGrid() {
      this.outerCellInterval = setInterval(
        this.moveGridClockwise,
        this.gridMovingInterval
      );
    },
    stopMovingGrid() {
      this.revolutions = 0;
    ...
    startCellInterval() {
      this.centerCellInterval = setInterval(
        this.randomizeCenterCell,
        15 * this.gridMovingInterval
      );
    },
    stopCellInterval() {
      clearInterval(this.centerCellInterval);
