  ...
  <div class="edit">
    ...
    <div class="content">
      ...
      <div class="input-container locations">
        ...
        <div
          class="search-field"
          :class="{ ignored: ignoreLocations }"
          v-click-outside="() => (searchResults = [])"
        >
          <div class="field">
            <input
              type="text"
          ...
          >
            ...
            <div v-else class="results">
              ...
              >
                <div
                  class="icon"
                  :style="{ backgroundColor: result.icon_background_color }"
                >
        ...
        <div class="choosen-locations" :class="{ ignored: ignoreLocations }">
          ...
          >
            ...
            <div class="radius">
              ...
              <div class="units">
                <p
                  :class="{ selected: location.radiusUnit === 'km' }"
                  @click="setUnitForLocation(location, 'km')"
                >
                  Km
                </p>
                <p
                  :class="{ selected: location.radiusUnit === 'mi' }"
                  @click="setUnitForLocation(location, 'mi')"
                >
                  Mi
                </p>
              </div>
      ...
      <div class="action-selection">
        <div class="radio-button" @click.stop="setAction('hyperLink')">
          <div
            ...
            :class="{ selected: action === 'hyperLink' }"
          ></div>
        ...
        <div class="radio-button" @click.stop="setAction('application')">
          <div
            class="radio-button_circle"
            :class="{ selected: action === 'application' }"
          ></div>
      ...
      <div class="actions">
        ...
        <div
          v-if="action === 'hyperLink' || action === 'image'"
        ...
        <div
          v-if="action === 'application'"
          class="input-container application"
        >
          <p>Select one of the application embeds here</p>
          <simple-dropdown
            :items="APPLICATION_NAMES ? APPLICATION_NAMES : []"
            :selectedItem="applicationEmbedName"
            :placeholderText="'Please select an application'"
            @selected-item="applicationEmbedName = $event"
          />
        </div>
      </div>
      ...
      <div v-if="isCenterImage" class="platform-info">
        ...
        <p @click="isMainImageForDomain = !isMainImageForDomain">
          Main image for domain: <span>{{ domain }}</span>
        </p>
      </div>

      <div class="clickThru-info">
        <p>
          Total clicks from <strong>outer</strong> cell:
          {{ outerClickThruCount }}
        </p>
        <p>
          Total clicks from <strong>center</strong> cell:
          {{ centerClickThruCount }}
        </p>
      </div>

      <button :class="{ inactive: !canSubmit }" @click="edit">
        Update this ad
      </button>
